  ...
  <div class="Workbench flex justify-between p-16px !w-full">
    <div class="Workbench-wrap-left">
      ...
      <div class="mb-[10px] rounded-[8px] bg-[#fff]">
        ...
        <div class="patient-manage-board flex justify-between">
          <div class="patient-manage-item">
            <div class="patient-manage-name">
              昨日新增患者
            </div>
            <div class="patient-manage-num">
              {{ panelData.yesterdayAddPatientNumber || 0 }}
            </div>
          </div>
          <div class="patient-manage-item">
            <div class="patient-manage-name">
          ...
          <div class="patient-manage-item">
            ...
            <div class="patient-manage-num">
              {{ panelData.totalPatientNumber || 0 }}
            </div>
          </div>
        </div>
        <div class="px-[10px]">
          ...
          <div class="py-[18px] !w-full">
            <NewPatientsLineChart width="100%" :chart-data="lineChartData" height="255px" />
          </div>
        </div>
      </div>
      <div class="mb-[10px] rounded-[8px] bg-[#fff]">
        ...
        <div class="todo-list flex justify-between">
          <div class="todo-item bg-orange" @click="jumpPage('/patient/center')">
            <div class="todo-item-name">
              待审核入组
            </div>
            <div class="todo-item-num">
              {{ panelData.applyGroup || 0 }}
            </div>
            <el-icon class="todo-item-icon text-[48px]">
              <svg-icon name="workbench-audit" />
            </el-icon>
          </div>
          <div class="todo-item bg-green" @click="jumpPage('/patient/chat')">
            <div class="todo-item-name">
              聊天室消息
            </div>
            <div class="todo-item-num">
              {{ panelData.chatRoomMessageNumber || 0 }}
            </div>
            <el-icon class="todo-item-icon text-[48px]">
          ...
          <div class="todo-item bg-pink" @click="jumpPage('/abnormal/list')">
            ...
            <div class="todo-item-num">
              {{ panelData.quotaAbnormalNumber || 0 }}
            </div>
            <el-icon class="todo-item-icon text-[48px]">
          ...
          <div class="todo-item bg-pink" @click="jumpPage('/abnormal/list')">
            ...
            <div class="todo-item-num">
              {{ panelData.questionnaireNumber || 0 }}
            </div>
            <el-icon class="todo-item-icon text-[48px]">
        ...
        <div class="mb-[15px] mt-[10px] flex items-center px-[15px] text-[14px] text-[#333] font-[400]">
          <div class="flex flex-1 items-center">
            <el-icon class="text-[28px]">
              <svg-icon name="workbench-server-patient" />
            </el-icon>
            <span class="ml-[10px]">服务患者数</span>
          </div>
          <div class="w-[80px]">
            {{ calendarDayData.servicePatientNumber || 0 }}
          </div>
        </div>
      ...
      <div class="mb-[10px] rounded-[8px] bg-[#fff]">
        ...
        <div class="relative">
          <div class="flex items-center p-[15px] pl-0 text-[16px] font-[550]">
            <span class="title-left-line" />
            <span>患者路径达标率</span>
            <el-icon v-if="patientPlanList?.length" class="ml-[12px] cursor-pointer text-[16px]" @click="handleSortChange">
              <svg-icon v-if="sortUp" name="workbench-sort-up" />
              <svg-icon v-else name="workbench-sort-down" />
            </el-icon>
            <el-icon v-if="!patientPlanList?.length" class="ml-[12px] cursor-pointer text-[16px]">
            ...
            <el-popover
              ...
              effect="dark"
              trigger="hover"
              content="用于展示计划向患者发送日程后，患者对日程的完成比率，数字越大代表患者对日程的完成情况越好。"
            >
              <template #reference>
          ...
          <div v-if="patientPlanList?.length" class="compliance-rate">
            <div v-for="(item, index) in patientPlanList" :key="index" class="compliance-item flex items-center">
              <span class="sort-index" :class="{ 'top-sort': index < 3 }">
                {{ index + 1 + (pageInfo.pageIndex - 1) * 10 }}
              </span>
              <span class="compliance-item-name" title="胸外科病房">{{ item.patientName }}</span>
              <span>{{ (item.completionScale * 100).toFixed(0) }}%</span>
            </div>
          </div>
        ...
        <!-- 分页 -->
        <div v-if="patientPlanList?.length" class="px-[30px] py-[40px]">
          <el-pagination
            background
            size="small"
...
import NewPatientsLineChart from './components/charts/NewPatientsLineChart.vue'
