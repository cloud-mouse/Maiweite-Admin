 ...
 */
export function createProxy(list: ProxyList = [], VITE_OPEN_PROXY: string) {
  ...
  for (const [prefix, target] of list) {
    ...
    ret[prefix] = {
      ...
      rewrite: path => VITE_OPEN_PROXY === 'true' ? path.replace(new RegExp(`^${prefix}`), `${prefix}`) : path.replace(/\/proxy/, ''),
